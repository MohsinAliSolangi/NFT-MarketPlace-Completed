...
it ("This is Auction function call: ", async function(){
    ...
    let itmes = await Market.connect(per2).createAuction(NFT.address, 1, 8, 30);
    balance = await NFT.balanceOf(per2.address);
    console.log("After puchase nft balance", balance);   
});
...
it("this is getLastTime funtion from auction: ", async function (){
  let itmes = await Market.getLastTime(3);

  console.log("%%%%%%%%%%%", itmes);   
});


it("this is bid funtion call per1 side: ", async function (){

  let bid = await Market.bid(3,{value:10})

})

it("this is bid funtion call per3 side: ", async function (){

  let bid = await Market.connect(per3).bid(3,{value:11})
...
it("this is bid funtion call per1 side: ", async function (){
  ...
  let bid = await Market.bid(3,{value:2})
...
it("this is conclude option function call :", async function (){
  ...
  let consludeAuction = await Market.concludeAuction(3,per1.address,)
})  
